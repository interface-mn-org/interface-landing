...
export function SplitFlapMuteToggle({ className = "" }: { className?: string }) {
  ...
  const t = useTranslations("Common.audio")
  ...
  return (
    <button
      ...
      aria-label={audio.isMuted ? t("unmuteAria") : t("muteAria")}
    >
      {audio.isMuted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
      <span>{audio.isMuted ? t("off") : t("on")}</span>
    </button>
  )
}
  ...
  if (isSpace) {
    return (
      <div
        style={{
          width: "0.3em",
          fontSize: "var(--splitflap-font-size, clamp(4rem, 15vw, 14rem))",
        }}
      />
    )
  }
